...
describe("App", () => {
  ...
  it("imports Schwab US transactions through the portfolio CSV parser", async () => {
    const user = userEvent.setup();
    vi.mocked(parsePortfolioCsv).mockReturnValueOnce({
      source: "schwab",
      dividends: [],
      warnings: [],
      trades: [
    ...
    await user.upload(
      screen.getByLabelText(/import csv/i),
      new File(["schwab"], "Individual_XXX882_Transactions.csv", { type: "text/csv" })
    );

    await waitFor(() => {
      expect(parsePortfolioCsv).toHaveBeenCalled();
    });

    expect(screen.getAllByText(/SNOWFLAKE INC CLASS A/i).length).toBeGreaterThan(0);
    expect(screen.getByText(/^SNOW$/)).toBeInTheDocument();
  });

  it("defaults Schwab imports to USD, earliest trade start date, and S&P 500 benchmark mode", async () => {
    const user = userEvent.setup();
    vi.mocked(parsePortfolioCsv).mockReturnValueOnce({
      source: "schwab",
      dividends: [
    ...
    vi.mocked(fetchHistoryForHoldings).mockResolvedValueOnce({
      "SNOW::US": [
        ...
        { date: "2026-06-29", close: 250 }
      ]
    });
    vi.mocked(fetchBenchmarks).mockResolvedValueOnce({
      topix: [
        { date: "2024-01-05", value: 1000, normalized: 100, source: "Yahoo Finance ^GSPC" },
        { date: "2026-06-29", value: 1500, normalized: 150, source: "Yahoo Finance ^GSPC" }
      ],
      nikkei225: []
    });
    ...
    expect(screen.getByLabelText(/track from/i)).toHaveValue("2024-01-05");
    expect(screen.getByText(/^USD$/)).toBeInTheDocument();
    expect(screen.getAllByText(/\$2,124/).length).toBeGreaterThan(0);
    expect(screen.getAllByText(/\$16\.80/).length).toBeGreaterThan(0);

    await user.click(screen.getByRole("button", { name: /refresh quotes/i }));

    await waitFor(() => {
      expect(fetchBenchmarks).toHaveBeenCalledWith("1y", "us");
    });
    expect(fetchQuotesForHoldings).toHaveBeenCalledWith(
      [expect.objectContaining({ code: "SNOW", market: "US", currency: "USD" })],
      "USD"
    );
    ...
    expect(screen.queryByText(/Nikkei 225/)).not.toBeInTheDocument();
  });
